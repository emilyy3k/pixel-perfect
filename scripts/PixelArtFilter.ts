...
export class PixelPerfectFilter extends PIXI.Filter {
  ...
  filterCount: number = 0; // Number of filters applied to the target sprite
  ...
  constructor(filterTarget: Token | Tile) {
    ...
    if (filterTarget?.texture) {
      ...
      console.warn(
        `pixel-perfect: No texture found for placeable ${filterTarget.id}, skipping filter application.`
      );
    }
  ...
  updateSpriteData(spriteMesh: PrimarySpriteMesh): void {
    ...
    if (!tex.valid) {
      console.warn(
        `pixel-perfect: Texture is not valid for sprite mesh ${spriteMesh}, skipping filter application.`
      );
      return;
    }
  ...
  updatePlaceableData(placeable: Token | Tile): void {
    ...
    if (!spriteMesh) {
      console.warn(
        `pixel-perfect: No sprite mesh found for placeable ${placeable.id}, skipping filter application.`
      );
      return;
    }
  ...
  apply(
    ...
    const needsUpdate =
      this.targetSprite.transform._worldID !== this.lastWorldID ||
