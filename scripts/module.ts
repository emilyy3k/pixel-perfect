...
Hooks.on("canvasReady", async () => {
  ...
  const applyFilterToPlaceable = (placeable: Token | Tile) => {
    if (!placeable?.mesh) {
      console.log(
        "pixel-perfect: Placeable has no sprite or texture",
        placeable?.id
      );
      return;
    }
    ...
    if (pixelFilter?.targetSprite && pixelFilter.targetSprite !== targetObj) {
      if (pixelFilter.targetSprite.filters) {
        pixelFilter.targetSprite.filters =
          pixelFilter.targetSprite.filters.filter((f) => f !== pixelFilter);
    ...
    if (!pixelFilter) {
      console.log(
        `pixel-perfect: Creating new filter for placeable ${placeable.id}`
      );
    ...
    const hasFilter =
      targetObj.filters?.some((f) => f === pixelFilter) || false;
    if (!hasFilter) {
      console.log(
        `pixel-perfect: Adding filter to placeable sprite ${placeable.id}`
      );

      // Ensure the filter is at the start of the filters array
      targetObj.filters.unshift(pixelFilter);
      console.log(
        `pixel-perfect: Filter added, total filters: ${targetObj.filters.length}`
      );
      if (placeable.texture?.baseTexture) {
        // using linear to avoid jittering. end result will still be sharp.
        placeable.texture.baseTexture.setStyle(
      ...
      if (filterIndex > 0) {
        console.log(
          `pixel-perfect: Moving filter to front for placeable ${placeable.id}`
        );
  ...
  const deleteFilterInstance = (placeable: Token | Tile) => {
    ...
    if (filter) {
      console.log(
        `pixel-perfect: Deleting filter instance for placeable ${placeable.id}`
      );
  ...
  Hooks.on("canvasTearDown", () => {
    ...
    for (const [id, filter] of pixelFilters) {
      ...
      if (spriteMesh) {
        ...
        spriteMesh.filters =
          spriteMesh.filters?.filter(
            (f) => !(f instanceof PixelPerfectFilter)
          ) || null;
        // If there are no filters left, set it to null for memory efficiency reasons (according to PIXI docs)
